...
export default function LoginPage() {
  ...
  return <main className="shell" style={{ maxWidth: 540, paddingTop: "12vh" }}><div className="card stack"><div><span className="brand">tutor<i>ly</i></span><h1 style={{ fontSize: 42, marginTop: 20 }}>Знайди свого репетитора</h1><p className="muted">Вхід потрібен, щоб захистити контакти та приватність викладачів.</p></div>{!configured && <p className="notice">Supabase ще не підключено. Зверніться до власника сайту.</p>}{error && <p className="error" role="alert">{error}</p>}<button className="button primary" disabled={Boolean(loading) || !configured} onClick={() => signIn("google")}>{loading === "google" ? "Переадресація…" : "Продовжити з Google"}</button><button className="button" disabled={Boolean(loading) || !configured} onClick={() => signIn("apple")}>{loading === "apple" ? "Переадресація…" : "Продовжити з Apple"}</button><p className="muted">Продовжуючи, ви погоджуєтеся користуватися Tutorly відповідально.</p></div></main>;
}
